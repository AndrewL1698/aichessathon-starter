...
v4.1 is in the shipped table with its PR open rather than merged, because the row that clears
zero is the fast control's and the platform proxy's does not; `docs/BENCH_LOG.md` has both.
...
| v4.1 | 2026-09-09 | `search/null-move-v4` (PR open, not merged) | Null-move pruning switched on: `NULL_MOVE_PRUNING = True`, the conditions and PR #10's repetition-path fix unchanged. Search only; the evaluation and the tables are byte-identical to v4.0. | Cycles 1 to 3 measured it as noise on the python-chess engine at 60k nodes/s. v4.0 searches at 1.3M with a learned leaf, where a ply costs twice what it did, so a node saved is worth twice as much. | Bench vs v4.0: 200 games **56.8%** (+88 =51 -61), Elo **+47**, interval **+6 to +90** -- the first pruning row this project has with a lower bound clear of zero. **But 45 s proxy 96 games 46.4% (-25, -86 to +34)** and no depth gain at 120 s (8.65 / 7.70 against v4.0's 8.43 / 7.85), so the gain is a fast-clock effect and the merge is a judgement call, not a formality. Two 120 s games both won. 0 / 0 / 0 / 0 over 296 games, worst overshoot 1 ms. Suite 9 of 17, same as v4.0. |
